...
def run():
    ...
    for host, ip_prefix, num_links in ((server, "10", 2), (client, "11", 2)):
    ...
    # rate limit links
    server.cmd("tc qdisc add dev server-eth0 root netem rate 750kbit")
    server.cmd("tc qdisc add dev server-eth1 root netem rate 1000kbit")
